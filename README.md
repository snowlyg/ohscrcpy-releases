...
- **设备操作工具**：支持安装 HAP/APK、打开 SSH 命令行、使用 SFTP 传输文件和
  重启设备。
- **设备信息管理**：支持自定义名称、设备分组、收藏排序和设备信息编辑；添加或
  修改设备时会阻止重复 IP，避免同一网络设备重复保存。
...
添加设备时可以同时填写可选名称，并从已有分组中选择或直接输入新分组。设备
菜单中的“编辑设备”支持修改名称、分组和离线设备地址；名称允许重复，网络设备
IP 全局唯一。远程中的设备仍可修改名称和分组，但地址会保持锁定直到会话结束。
